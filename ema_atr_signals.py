...
def fetch_klines(symbol, interval=TIMEFRAME, limit=BINANCE_KLINES_LIMIT):
    ...
    try:
        ...
        # Convert to numeric - including volume for calculations
...
def groq_chat_reply(user_text):
    ...
    if not GROQ_API_KEY:
        return "AI chat is currently unavailable. Groq API key not configured."
    ...
    try:
        ...
        if "error" in data:
            ...
            return "Sorry, I encountered an AI API error. The trading signals will continue working."
        
        return "Sorry, I couldn't get a proper response from AI. Trading signals are still active."
        
    except Exception as e:
        print(f"[Groq] error: {e}")
        return "AI chat is temporarily unavailable. Trading signals continue to work normally."
...
def validate_groq_api_key():
    ...
    try:
        ...
        r = requests.post(GROQ_CHAT_URL, json=payload, headers=headers, timeout=10)
...
def main():
    ...
    # Validate required environment variables (core functionality)
    required_vars = {
    ...
    if GROQ_API_KEY:
        print("[Groq] Testing API key...")
        is_valid, message = validate_groq_api_key()
        if is_valid:
            print("[Groq] ✅ API key is valid - AI chat enabled")
        else:
            print(f"[Groq] ❌ API key validation failed: {message}")
            print("[Groq] AI chat will be disabled. Trading signals will still work.")
    else:
        print("⚠️  GROQ_API_KEY not set - AI chat will be unavailable (trading signals will work)")
